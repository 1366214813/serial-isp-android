...
object UsbDeviceManager {
    ...
    fun getDriverForDevice(device: UsbDevice): UsbSerialProber {
        return when {
            ...
            PL2303_VIDS.contains(device.vendorId) -> ProlificSerialProber.getProbeInstance()
            FTDI_VIDS.contains(device.vendorId) -> FtdiSerialProber.getProbeInstance()
            else -> DefaultProber.getProbeInstance()
        }
    }
